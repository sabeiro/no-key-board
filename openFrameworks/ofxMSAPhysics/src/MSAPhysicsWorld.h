...
namespace msa {
  ...
  namespace physics {
    ...
    public:
      ...
      WallT<T>* makeWall(T P1, T P2, T P3);
      ...
      WallT<T>* getWall(long i);

      SectorT<T>* getSector(long i);
      ...
      WorldT<T>* setCutOff(float cf = 0.); // set the cut off for connecting the particles
      WorldT<T>* setGravity(float gy = 0); // set gravity (y component only)
      WorldT<T>* setGravity(T g); // set gravity (full vector)
      WorldT<T>* setWalls(); // set gravity (full vector)
      T& getGravity();
    ...
    //--------------------------------------------------------------
    template <typename T>
      WallT<T>* WorldT<T>::makeWall(T P1, T P2,T P3) {
      WallT<T> *w = new WallT<T>(P1,P2,P3);
      addWall(w);
      return w;
    }
    ...
    //--------------------------------------------------------------
    template <typename T>
      WallT<T>* WorldT<T>::addWall(WallT<T> *w) {
      _walls.push_back(w);
      return w;
    }
    ...
    //--------------------------------------------------------------
    template <typename T>
      WallT<T>*		WorldT<T>::getWall(long i) {
      return i < numberOfWalls() ? _walls[i] : NULL;
    }
	
    //--------------------------------------------------------------
    template <typename T>
      SectorT<T>*		WorldT<T>::getSector(long i) {
      return i < _sectors.size() ? _sectors[i] : NULL;
    }
    ...
    //--------------------------------------------------------------
    template <typename T>
      long WorldT<T>::numberOfWalls() {
      return _walls.size();
    }
		
    //--------------------------------------------------------------
    template <typename T>
      long WorldT<T>::numberOfSectors() {
    ...
    //--------------------------------------------------------------
    template <typename T>
      WorldT<T>*  WorldT<T>::setCutOff(float cf) {
      params.cutOff = cf;
      params.doConnections = cf > 0.;
      return this;
    }
	...
	if(particle->_isDead) {							// if particle is dead
	  ...
	  typename vector<WallT<T>*>::iterator itw = _walls.begin();
	  /* particle->addGravity(); */
	  /* if(params.doWalls){ */
	  /*   while(itw != _walls.end()) { */
	  /*     WallT<T>* wall = *itw; */
    ...
    //--------------------------------------------------------------
    template <typename T>
      void WorldT<T>::updateWalls() {
      typename vector<WallT<T>*>::iterator itw = _walls.begin();
      while(itw != _walls.end()) {
	WallT<T>* wall = *itw;
	typename vector<ParticleT<T>*>::iterator itp = _particles.begin();
	while(itp != _particles.end()){
	  ParticleT<T>* particle = *itp;
	  wall->impact(particle);
	  itp++;
	}
	itw++;
    ...
    template <typename T>
      ...
      int ns = _sectors.size();
      for(int s=0; s<ns; s++) {
	_sectors[s]->checkSectorCollisions();
	_sectors[s]->clear();
      }
    }
